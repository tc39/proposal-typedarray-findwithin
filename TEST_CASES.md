...
- [ ] [2.9.3] `new Uint8Array([]).search('')` → TypeError
...
### 7.2 BigInt array with iterable needle

- [ ] [7.2.1] `new BigInt64Array([1n, 2n, 3n]).search([2n, 3n])` → 1
- [ ] [7.2.2] `new BigUint64Array([1n, 2n, 3n]).search([2n, 3n])` → 1

### 7.3 Cross-type with BigInt

- [ ] [7.3.1] `new BigInt64Array([1n, 2n]).search(new BigUint64Array([1n, 2n]))` → 0 (both yield BigInts)
- [ ] [7.3.2] `new BigInt64Array([1n, 2n]).search(new Uint8Array([1, 2]))` → -1 (Uint8Array yields Numbers, BigInt64Array expects BigInts)

---

## 8. Evaluation Order and Observable Side Effects

### 8.1 ValidateTypedArray before needle collection

For each method (`search`, `searchLast`):

- [ ] [8.1.1] Detached buffer with an iterable needle that has observable side effects → TypeError from validation, iterator never called
- [ ] [8.1.2] Detached buffer with a TypedArray needle → TypeError from haystack validation, needle's buffer not accessed

### 8.2 Needle collection before ValidateIntegralNumber
...
### 8.3 ValidateIntegralNumber after needle collection

For each method (`search`, `searchLast`):

- [ ] [8.3.1] Valid TypedArray, valid iterable needle, invalid position → error from position validation
  ```js
  new Uint8Array([1,2,3]).search([1,2], NaN) // → RangeError
  new Uint8Array([1,2,3]).search([1,2], 'bad') // → TypeError
  ```

### 8.4 Iterator side effects (iterable path only)

For each method (`search`, `searchLast`):

- [ ] [8.4.1] Iterable whose iterator modifies global state → verify iteration happens exactly once
- [ ] [8.4.2] Iterable whose iterator throws midway → error propagates, partial iteration observable
- [ ] [8.4.3] TypedArray needle does NOT trigger iterator side effects → @@iterator is not called (see also 2.12.3)
...
- [ ] [10.1.1] `new Uint8Array([5]).search([5])` → 0
- [ ] [10.1.2] `new Uint8Array([5]).search([6])` → -1
- [ ] [10.1.3] `new Uint8Array([5]).searchLast([5])` → 0

### 10.2 Needle same length as haystack

- [ ] [10.2.1] `new Uint8Array([1,2,3]).search([1,2,3])` → 0
- [ ] [10.2.2] `new Uint8Array([1,2,3]).search([1,2,4])` → -1
- [ ] [10.2.3] `new Uint8Array([1,2,3]).searchLast([1,2,3])` → 0

### 10.3 Overlapping pattern in haystack

- [ ] [10.3.1] `new Uint8Array([1,1,1,2]).search([1,1,2])` → 1
- [ ] [10.3.2] `new Uint8Array([1,1,1,1]).search([1,1])` → 0
- [ ] [10.3.3] `new Uint8Array([1,1,1,1]).searchLast([1,1])` → 2

### 10.4 Repeated single value

- [ ] [10.4.1] `new Uint8Array([0,0,0,0,0]).search([0,0])` → 0
- [ ] [10.4.2] `new Uint8Array([0,0,0,0,0]).searchLast([0,0])` → 3
- [ ] [10.4.3] `new Uint8Array([0,0,0,0,0]).search([0,0], 2)` → 2
- [ ] [10.4.4] `new Uint8Array([0,0,0,0,0]).searchLast([0,0], 2)` → 2
...
- [ ] [10.6.1] `new Uint8Array([1,2,3]).search([], 3)` → 3 (empty needle returns position)
- [ ] [10.6.2] `new Uint8Array([1,2,3]).search([1], 3)` → -1 (position + needleLength > haystackLength)
...
- [ ] [10.7.1] `new Uint8ClampedArray([255, 0]).search([255])` → 0
- [ ] [10.7.2] `new Uint8ClampedArray([255, 0]).search([300])` → -1 (300 does not SameValueZero-equal 255)
- [ ] [10.7.3] `new Uint8ClampedArray([255, 0]).search(new Uint8ClampedArray([255]))` → 0 (iterated value is 255, matches stored 255)

### 10.8 Typed integer overflow boundaries

- [ ] [10.8.1] `new Uint8Array([255, 0, 1]).search([255, 0])` → 0
- [ ] [10.8.2] `new Int8Array([-128, 127]).search([-128, 127])` → 0
- [ ] [10.8.3] `new Uint16Array([65535, 0]).search([65535, 0])` → 0
- [ ] [10.8.4] `new Int32Array([-2147483648, 2147483647]).search([-2147483648])` → 0
...
- [ ] [10.9.2] `Uint8Array.prototype.search.call(new Int32Array([1,2,3]), new Uint8Array([2,3]))` → 1 (needle is different-type, iterated via @@iterator)

### 10.10 Empty haystack with non-empty needle (both directions)

- [ ] [10.10.1] `new Uint8Array([]).search([1])` → -1
- [ ] [10.10.2] `new Uint8Array([]).searchLast([1])` → -1
...
### 11.1a Iterable needle backed by SharedArrayBuffer (snapshotted via @@iterator)

- [ ] [11.1a.1] Non-TypedArray iterable backed by a SharedArrayBuffer (e.g., a custom iterable wrapping SAB reads) — elements are snapshotted via `@@iterator` in `TypedArraySubsequenceFromIterable`. Concurrent modifications after iteration completes do not affect the search.
...
- [ ] [11.3.1] For TypedArray needles: both needle and haystack elements are read live (neither is snapshotted). Another agent may modify either during the search. Users must synchronize access externally.
- [ ] [11.3.2] For iterable needles: the needle is snapshotted via `@@iterator` before the search. The haystack is read live. The search compares a frozen needle List against live haystack reads.
...
- [ ] [12.2.1] Auto-length needle TypedArray over resizable buffer, no resize → normal iteration, produces correct snapshot List
- [ ] [12.2.2] Fixed-length needle TypedArray over resizable buffer, no resize → normal iteration, produces correct snapshot List

### 12.3 Needle iteration detaches the haystack's buffer (iterable path only)
...
- [ ] [12.3.1] Custom iterable that detaches the haystack's ArrayBuffer during iteration → search returns -1 (element reads return `undefined`)
  ```js
  ...
  ```
- [ ] [12.3.2] Same scenario with `searchLast` → -1

### 12.4 Needle TypedArray's own buffer shrunk or detached before read
...
- [ ] [12.4.1] Needle is auto-length over a resizable buffer; buffer shrunk below the needle's range before calling the method → TypeError from `IsTypedArrayOutOfBounds`
- [ ] [12.4.2] Needle is fixed-length over a resizable buffer; buffer shrunk below `byteOffset + length * elementSize` before calling the method → TypeError from `IsTypedArrayOutOfBounds`
- [ ] [12.4.3] Needle TypedArray's buffer is detached before calling the method → TypeError from `IsTypedArrayOutOfBounds`

### 12.5 Needle TypedArray's buffer grown (auto-length needle)
...
- [ ] [12.5.1] Auto-length needle over a resizable buffer; buffer is grown before calling the method → needle length reflects the new buffer size; search uses all elements

### 12.6 Haystack buffer resized during needle iteration (iterable path only)
...
- [ ] [12.6.1] Haystack is auto-length; buffer is grown during needle iteration → `TypedArrayLength(_taRecord_)` returns the pre-growth length (witness is stale); search only scans the original range. Elements beyond the original length are not searched.
- [ ] [12.6.2] Haystack is auto-length; buffer is shrunk during needle iteration (but TypedArray remains in bounds) → `TypedArrayLength(_taRecord_)` returns the pre-shrink length (witness is stale); search may read `undefined` for elements beyond the new length. Those reads will not match, effectively returning -1 for needles that would have matched only in the now-truncated region.
- [ ] [12.6.3] Haystack is auto-length; buffer is shrunk below the haystack's byte offset during needle iteration → `TypedArrayLength(_taRecord_)` uses stale witness; search may read `undefined` for all elements. Returns -1.
- [ ] [12.6.4] Haystack is fixed-length; buffer is shrunk below the fixed range during needle iteration → same stale-witness behaviour; element reads return `undefined`.

### 12.7 Both haystack and needle backed by resizable ArrayBuffers

For each method (`search`, `searchLast`):

- [ ] [12.7.1] Both are auto-length over separate resizable buffers, no resize during operation → normal behaviour
- [ ] [12.7.2] Both share the same resizable ArrayBuffer (different views); no resize → needle is snapshotted via iteration, search proceeds normally
- [ ] [12.7.3] Both share the same resizable ArrayBuffer; buffer is grown during needle iteration → needle snapshot may include new elements; haystack length uses stale witness
